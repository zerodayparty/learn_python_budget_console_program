...
DATE_PATTERN = re.compile(r"^\d{4}-\d{2}-\d{2}$")  # 날짜가 YYYY-MM-DD 모양인지 검사할 규칙이다.
MONTH_PATTERN = re.compile(r"^\d{4}-\d{2}$")  # 월이 YYYY-MM 모양인지 검사할 규칙이다.
ALLOWED_TYPES = {"income", "expense"}  # 거래 타입으로 허용하는 두 값을 모아 둔다.
...
def validate_date(value: str) -> str:  # 날짜 문자열을 검사하고 올바른 값을 돌려준다.
    ...
    try:  # 실제 달력에 존재하는 날짜인지 확인을 시도한다.
        datetime.strptime(cleaned, "%Y-%m-%d")  # 문자열을 연도-월-일 날짜로 바꿔 본다.
...
def validate_month(value: str) -> str:  # 월 문자열을 검사하고 올바른 값을 돌려준다.
    ...
    try:  # 01부터 12까지의 실제 월인지 확인을 시도한다.
        datetime.strptime(cleaned, "%Y-%m")  # 문자열을 연도-월 값으로 바꿔 본다.
